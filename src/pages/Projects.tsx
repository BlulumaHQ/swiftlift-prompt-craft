      ...
      <div className="flex flex-col h-screen">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 max-w-5xl mx-auto w-full">
          ...
          <div className="mb-4 sm:mb-6">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground mt-1 break-all">{selected.sourceUrl} · {selected.packageTier} · {selected.dateCreated}</p>
          </div>
          <div className="flex flex-wrap gap-2 mb-4 sm:mb-6">
            ...
            <button className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs sm:text-sm font-medium bg-secondary text-secondary-foreground opacity-50 cursor-not-allowed">
              <Eye size={14} /> Preview
            </button>
            <button className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs sm:text-sm font-medium bg-secondary text-secondary-foreground opacity-50 cursor-not-allowed">
            ...
            {[{ title: tierLabel.a, content: promptA, field: 'a' }, { title: tierLabel.b, content: promptB, field: 'b' }].map(p => (
              <div key={p.field} className="prompt-output">
                ...
                <div className="prompt-output-body">
                  <pre className="whitespace-pre-wrap break-words text-xs sm:text-sm">{p.content || 'No prompt generated yet.'}</pre>
                </div>
              </div>
            ))}
          </div>
              ...
              <div key={p.id} className="panel-section hover:shadow-md transition-shadow relative group">
                <button onClick={() => setSelected(p)} className="text-left w-full">
                  ...
                  <div className="flex items-center gap-2 mt-3 flex-wrap">
                    ...
                    {p.modules.length > 3 && <span className="text-xs text-muted-foreground">+{p.modules.length - 3}</span>}
                  </div>
